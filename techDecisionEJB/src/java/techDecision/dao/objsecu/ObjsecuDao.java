 ...
 * @version 0.1
 */
@Stateless(mappedName="ejbObjsecu")
public class ObjsecuDao implements IObjsecuDaoLocal, IObjsecuDaoRemote {
    @PersistenceContext(unitName="techDecisionEJBPU")
    private EntityManager em;
    private ObjsecuJpaSimple objS = null;
    ...
    public Objsecu findObjsecu(Integer id) {
        return objS.findObjsecu(id);
    }

    public List<Objsecu> findObjsecuEntities() {
        return objS.findObjsecuEntities();
    }

    public List<Objsecu> findObjsecuEntities(int maxResults, int firstResult) {
        return objS.findObjsecuEntities(maxResults, firstResult);
    }

    public Long getObjsecuCount() {
        return objS.getObjsecuCount();
    }

    public void init() {
        objS = new ObjsecuJpaSimple(em);
    }
