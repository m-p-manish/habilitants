 ...
 * @version 0.4
 */
@Stateless(mappedName="ejbObjsAttrs")
public class ObjsAttrsDao implements IObjsAttrsDaoLocal, IObjsAttrsDaoRemote {
    @PersistenceContext(unitName="techDecisionEJBPU")
    private EntityManager em;
    private ObjsAttrsJpaSimple objS = null;

    public void init(){
        objS = new ObjsAttrsJpaSimple(em);
    }
    public void create(ObjsAttrs objsAttrs) throws PreexistingEntityException, RollbackFailureException, Exception {
        objS.create(objsAttrs);
    }
    ...
    public ObjsAttrs findObjsAttrs(Integer id) {
        return objS.findObjsAttrs(id);
    }

    public List<ObjsAttrs> findObjsAttrsEntities() {
        return objS.findObjsAttrsEntities();
    }

    public List<ObjsAttrs> findObjsAttrsEntities(int maxResults, int firstResult) {
        return objS.findObjsAttrsEntities(maxResults, firstResult);
    }

    public Long getObjsAttrsCount() {
        return objS.getObjsAttrsCount();
    }

    public List<ObjsAttrs> relatedAttrs(int idObjs) {
        return objS.relatedAttrs(idObjs);
    }

    public Boolean existNomAppli(String sNom) {
        return objS.existNomAppli(sNom);
    }
