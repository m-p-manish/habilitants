/*
Copyright Stéphane Georges Popoff, (mars - juillet 2009)
...
public class ObjsCpteJpaController {
    ...
    ObjsCpteJpaController(EntityManager emm) {
        em = emm;
    }
    ...
    public void create(int pk, int cpte, int objs) throws RollbackFailureException, Exception {
        try {
            ObjsCpte objsCpte = new ObjsCpte(pk);
            Compte c = em.getReference(Compte.class, cpte);
            Objsecu o = em.getReference(Objsecu.class, objs);
            objsCpte.setFkcpte(c);
            objsCpte.setFkobjs(o);
            em.persist(objsCpte);
        } catch (Exception ex) {
            throw ex;
        } finally {
